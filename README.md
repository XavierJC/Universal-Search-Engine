...
## 测试数据及测试结果
#### 测试案例 1：多文档检索与 TF (词频) 排序

* **测试目的：** 验证系统能否在多个文件中找到同一个词，并根据出现次数（Term Frequency）正确降序排列。
* **测试操作：** 输入关键词 `"hat"` (帽子)。
* **背景分析：** 在 `source_2.txt` (小王子) 中，叙述者画的一条蛇被大人误认为是 "hat" 。在 `source_1.txt` (包法利夫人) 中，详细描写了主角那顶怪异的 "billycock hat" 。
* **预期结果：** 系统应列出 `source_1.txt` 和 `source_2.txt`，且根据文件中 "hat" 出现的次数多少进行排序。
* **实际输出结果：**
```plaintext
>>> 搜索结果: "hat"
--------------------------------
...
```
* **结果分析：** 系统成功定位了两个文件，并正确统计了 `source_1.txt` 中 "hat" 出现频率更高（因为该文件用了大段篇幅描写帽子的细节），排序逻辑正确。

#### 测试案例 2：大小写不敏感 (Case Insensitivity)

* **测试目的：** 验证系统在索引和搜索时是否完成了“归一化”处理。
* **测试操作：** 输入关键词 `"STRESS"` (全大写)。
* **背景分析：** 在 `test.txt` 中，单词 "Stress" 在句首大写 ，而 "stress" 在句中以小写形式多次出现 。


* **预期结果：** 系统应忽略大小写差异，将 "Stress" 和 "stress" 均计入统计，并返回 `test.txt`。
* **实际输出结果：**
```plaintext
>>> 搜索结果: "STRESS"
--------------------------------
...
* **结果分析：** 尽管输入的是全大写，系统准确找到了 `test.txt`，且统计次数为 4 次，证明系统成功识别了原文中 "Stress" (Line 1, 7) 和 "stress" (Line 2, 6) 的所有形态。

#### 测试案例 3：动态数据加载与健壮性验证

* **测试目的：** 验证系统读取外部文件的能力。通过人为修改文件内容，证明系统不是在“假运行”。
* **测试操作：**
  1. **初次搜索：** 输入关键词 `"Algorithm"`。
     * **结果：** 提示 `未找到包含 "Algorithm" 的文档。` (当前所有文件中均未包含此词)。


   2. **数据注入：** 在 `test.txt` 文件的末尾手动增加一行文本：
       > "The underlying search Algorithm is efficient."


   3. **重启系统并再次搜索：** 再次输入 `"Algorithm"`。


* **预期结果：** 初次搜索失败，添加文本并重启后，系统应能立即索引到新内容。
* **实际输出结果 (重启后)：**
```plaintext
>>> 正在索引文件: [test.txt] ...
...
...
* **结果分析：** 该测试强有力地证明了系统实现了 **数据与代码分离**。系统通过 `fopen` / `fgets` 动态读取了被修改后的 `test.txt`，准确识别了新加入的测试文本。

#### 测试案例 4：边界测试 (标点符号处理)

* **测试目的：** 验证分词算法是否能剥离紧贴单词的标点符号。
* **测试操作：** 输入关键词 `"constrictor"`。
* 
**背景分析：** 在 `source_2.txt` 中，该词出现了两次，一次是 "Boa constrictors swallow..." ，另一次是 "...boa constrictor," (紧跟逗号) 。


* **预期结果：** 系统应能去除逗号，准确匹配单词。
* **实际输出结果：**
```plaintext
>>> 搜索结果: "constrictor"
--------------------------------
...
* **结果分析：** 系统正确处理了 `constrictor.` (句号后) 和 `constrictor,` (逗号后) 的情况，分词逻辑健壮。
